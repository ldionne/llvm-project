...
- The standard library hardening mode is now set to ``fast`` by default in translation units that do not enable
  compiler optimziations. The hardening mode can still be controlled explicitly by defining ``_LIBCPP_HARDENING_MODE``
  to another value.
