...
- The CMake option ``LIBCXX_INCLUDE_BENCHMARKS`` has been removed. The benchmarks are now part of the
  test suite and they are enabled whenever the tests are enabled (via ``LIBCXX_INCLUDE_TESTS``).
