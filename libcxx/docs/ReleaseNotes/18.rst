...
- libc++ no longer supports containers of ``const``-qualified element type,
  such as ``std::vector<const T>`` and ``std::list<const T>``. This used to be
  supported as an extension. Likewise, ``std::allocator<const T>`` is no longer
  supported. If you were using ``std::vector<const T>``, replace it with
