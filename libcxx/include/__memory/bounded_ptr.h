...
#ifndef _LIBCPP___MEMORY_BOUNDED_PTR_H
#define _LIBCPP___MEMORY_BOUNDED_PTR_H
...
public:
  ...
  _LIBCPP_ALWAYS_INLINE constexpr bounded_ptr(detail::nullptr_t) : base_(nullptr), count_(0), offset_(0) {}

  _LIBCPP_ALWAYS_INLINE constexpr explicit bounded_ptr() : bounded_ptr(nullptr) {}
  ...
  _LIBCPP_ALWAYS_INLINE explicit bounded_ptr(T* pointer, T const* begin, T const* end) {
    base_ = reinterpret_cast<CharType*>(const_cast<T*>(begin));

    // Store (end - begin) into count_, making sure we don't overflow
    if (__improbable(os_sub_overflow(reinterpret_cast<uintptr_t>(end), reinterpret_cast<uintptr_t>(begin), &count_))) {
    ...
    if (__improbable(
            os_sub_overflow(reinterpret_cast<uintptr_t>(pointer), reinterpret_cast<uintptr_t>(begin), &offset_))) {
  ...
  // desired, `reinterpret_pointer_cast` can be used instead.
  //
  ...
  _LIBCPP_ALWAYS_INLINE bounded_ptr(bounded_ptr<U, Policy> const& other)
      : base_(other.base_),
        count_(other.count_),
  ...
  // source `bounded_ptr`.
  template <typename U, typename Policy, typename = detail::enable_if_t<detail::is_convertible_v<U*, T*> > >
  _LIBCPP_ALWAYS_INLINE bounded_ptr& operator=(bounded_ptr<U, Policy> const& other) {
    base_   = other.base_;
    count_  = other.count_;
    offset_ = static_cast<int32_t>(reinterpret_cast<CharType*>(static_cast<T*>(other.get_ptr_())) - other.base_);
    return *this;
  ...
  _LIBCPP_ALWAYS_INLINE bounded_ptr& operator=(detail::nullptr_t) {
    *this = bounded_ptr();
    return *this;
  }
  ...
  _LIBCPP_ALWAYS_INLINE T_& operator*() const {
  ...
  _LIBCPP_ALWAYS_INLINE T* operator->() const {
    if (__improbable(!in_bounds_())) {
  ...
  template <typename T_ = T> // delay instantiation to avoid forming invalid ref for bounded_ptr<void>
  _LIBCPP_ALWAYS_INLINE T_& operator[](ptrdiff_t n) const {
    return *(*this + n);
  }
  ...
  _LIBCPP_ALWAYS_INLINE T* discard_bounds() const {
    if (__improbable(!in_bounds_())) {
      TrappingPolicy::trap(
          "bounded_ptr<T>::discard_bounds: Discarding the bounds on "
  ...
  _LIBCPP_ALWAYS_INLINE T* unsafe_discard_bounds() const { return get_ptr_(); }
  ...
  _LIBCPP_ALWAYS_INLINE explicit operator bool() const { return get_ptr_() != nullptr; }
  ...
  _LIBCPP_ALWAYS_INLINE bounded_ptr& operator++() {
    *this += 1;
    return *this;
  }
  _LIBCPP_ALWAYS_INLINE bounded_ptr operator++(int) {
  ...
  _LIBCPP_ALWAYS_INLINE bounded_ptr& operator+=(ptrdiff_t n) {
    ...
    if (__improbable(os_mul_overflow(n, sizeof(T), &bytes))) {
      TrappingPolicy::trap("bounded_ptr<T>::operator+=(n): Calculating the number of bytes to "
                           "add to the offset (n * sizeof(T)) would trigger an overflow");
    }
    if (__improbable(os_add_overflow(offset_, bytes, &offset_))) {
      TrappingPolicy::trap("bounded_ptr<T>::operator+=(n): Adding the specified number of bytes "
  ...
  _LIBCPP_ALWAYS_INLINE bounded_ptr& operator-=(ptrdiff_t n) {
    ...
    if (__improbable(os_sub_overflow(offset_, bytes, &offset_))) {
      TrappingPolicy::trap("bounded_ptr<T>::operator-=(n): Subtracting the specified number of bytes "
                           "from the offset representing the current position would overflow.");
    }
    return *this;
  }

  friend _LIBCPP_ALWAYS_INLINE bounded_ptr operator+(bounded_ptr p, ptrdiff_t n) {
    p += n;
    return p;
  }
  friend _LIBCPP_ALWAYS_INLINE bounded_ptr operator+(ptrdiff_t n, bounded_ptr p) {
    p += n;
    return p;
  }
  friend _LIBCPP_ALWAYS_INLINE bounded_ptr operator-(bounded_ptr p, ptrdiff_t n) {
  ...
  // operation.
  friend _LIBCPP_ALWAYS_INLINE ptrdiff_t operator-(bounded_ptr const& a, bounded_ptr const& b) {
    return a.get_ptr_() - b.get_ptr_();
  }

  friend _LIBCPP_ALWAYS_INLINE ptrdiff_t operator-(bounded_ptr const& a, T const* b) { return a.get_ptr_() - b; }

  friend _LIBCPP_ALWAYS_INLINE ptrdiff_t operator-(T const* a, bounded_ptr const& b) { return a - b.get_ptr_(); }

private:
  _LIBCPP_ALWAYS_INLINE bool in_bounds_() const {
  ...
  _LIBCPP_ALWAYS_INLINE T* get_ptr_() const {
  ...
  friend struct bounded_ptr; // for cross-type operations and conversions
...
_LIBCPP_ALWAYS_INLINE bool operator==(bounded_ptr<T, P1> const& a, bounded_ptr<U, P2> const& b) {
...
_LIBCPP_ALWAYS_INLINE bool operator==(bounded_ptr<T, P> const& a, U* b) {
  return a.unsafe_discard_bounds() == b;
}

template <typename T, typename P, typename U, typename = detail::WhenComparable<T*, U*> >
_LIBCPP_ALWAYS_INLINE bool operator==(U* a, bounded_ptr<T, P> const& b) {
...
_LIBCPP_ALWAYS_INLINE bool operator==(detail::nullptr_t, bounded_ptr<T, Policy> const& p) {
...
template <typename T, typename Policy>
_LIBCPP_ALWAYS_INLINE bool operator==(bounded_ptr<T, Policy> const& p, detail::nullptr_t) {
  return p.unsafe_discard_bounds() == nullptr;
}
...
_LIBCPP_ALWAYS_INLINE bool operator<(T* a, bounded_ptr<U, P> const& b) {
  return a < b.unsafe_discard_bounds();
}
...
_LIBCPP_ALWAYS_INLINE T* reinterpret_pointer_cast(U* p) noexcept {
...
_LIBCPP_ALWAYS_INLINE bounded_ptr<T, Policy> reinterpret_pointer_cast(bounded_ptr<U, Policy> const& p) noexcept {
  using CharType      = detail::copy_cv_t<T, char>;
  CharType* new_begin = reinterpret_cast<CharType*>(p.base_);
  CharType* new_end   = new_begin + p.count_;
  return bounded_ptr<T, Policy>(
      reinterpret_cast<T*>(p.get_ptr_()), reinterpret_cast<T const*>(new_begin), reinterpret_cast<T const*>(new_end));
}

_LIBCPP_END_NAMESPACE_STD

#endif // _LIBCPP___MEMORY_BOUNDED_PTR_H
